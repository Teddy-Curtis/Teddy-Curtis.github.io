---
layout: post
title: Simple multiprocessing dataloader
...
To get an idea of how this class works, and how the python multiprocessing library is used, a simplified version is shown in this post. 
...
4. We want to limit the number of batches that are pre-loaded to some user-defined limit, even if training slows for some reason. 

With these constraints in mind, let's start designing our class! First, lets define out dataset, and then show a non-multiprocessing dataloader to get an idea of our baseline. 
...
This takes in the number of total samples, `n_samples`, and when we indexed, it returns a data sample.
...
As a baseline, we will compare our final multiprocessing dataloader to a simple single-processor dataloader, called `BasicDataLoader`.
...
The second is called `out_q`, which holds the data batches that are output from the worker processes, and which the dataloader pull from. A schematic of this is shown below:
...
class MPBatchLoader:
    ...
    def __iter__(self) -> Iterator[np.ndarray]:
        ...
        # Limit how far ahead we allow workers to run.
        # You can tune this; PyTorch uses roughly prefetch_factor * num_workers.
        window = self.prefetch_batches  # simplest: tie it to out_q maxsize
        ...
        # Submit initial window of tasks
        # catch the case where n_total < window
        while submit_idx < min(window, n_total):
        ...
        try:
            while next_batch_idx < n_total:
                ...
                while next_batch_idx in reorder:
                    ...
                    # As we advance, submit one more task to keep the window full.
